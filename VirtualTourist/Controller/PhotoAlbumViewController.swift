...
class PhotoAlbumViewController: UIViewController {
    ...
    private var photosDetails = [PhotoDetails]()
    ...
    private func fetchPhotos(coordinate: CLLocationCoordinate2D) {
        ...
        FlickrClient.getPhotosList(latitude: coordinate.latitude, longitude: coordinate.longitude, completion: handleGetPhotosList(photosDetails:error:))
    }
    
    private func handleGetPhotosList(photosDetails: [PhotoDetails], error: Error?) {
    ...
    private func deletePhoto(indexPath: IndexPath) {
        ...
        if shouldDownload {
            photosDetails.remove(at: indexPath.item)
        }
...
extension PhotoAlbumViewController: UICollectionViewDataSource {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        if DataModel.photosData.count > indexPath.item {
            ...
            cell.downloadPhoto(for: photosDetails[indexPath.item], pin: pin)
        }
        return cell
    }
